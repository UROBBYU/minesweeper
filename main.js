...
msgBox.build = function (title, sub, input) {
	...
	for (let i = 0; i < sub.length; i++)
		this.children.submsg.appendChild(
			createElement(`<span style="color:${sub[i]}"">${sub[++i]}</span>`)
		)
	this.children.submsg.style.display =
		this.children.submsg.innerHTML === '' ? 'none' : ''

	this.children.input.innerHTML = ''

	this.children.input.appendChild(
		createElement('<span class="delimiter"></span>')
	)
	for (const arr of input) {
		let section = createElement('<span class="section"></span>')
		if (arr.type) {
			const elem = document.createElement(
				arr.type === 'number' ? 'input' : 'span'
			)
	...
	this.children.input.style.display =
		this.children.input.innerHTML === '' ? 'none' : ''
}
...
function updateCell(cell) {
	...
	cell.elem.children[0].innerText =
		cell.data.value === 0 ? '' : cell.data.value
...
function createCell(x, y, options) {
	...
	elem.innerHTML = `
        ...
        <div class="center" ${options.center ? '' : 'hidden=""'}>${
		options.type === 'flag' ? '🏴' : ''
	}</div>
        <div class="top-left${options.topLeft ? '' : ' inverted'}" ${
		options.top && options.left ? '' : 'hidden=""'
	}></div>
        <div class="top" ${options.top ? '' : 'hidden=""'}></div>
        <div class="top-right${options.topRight ? '' : ' inverted'}" ${
		options.top && options.right ? '' : 'hidden=""'
	}></div>
        <div class="right" ${options.right ? '' : 'hidden=""'}></div>
        <div class="bottom-right${options.bottomRight ? '' : ' inverted'}" ${
		options.bottom && options.right ? '' : 'hidden=""'
	}></div>
        <div class="bottom" ${options.bottom ? '' : 'hidden=""'}></div>
        <div class="bottom-left${options.bottomLeft ? '' : ' inverted'}" ${
		options.bottom && options.left ? '' : 'hidden=""'
	}></div>
		...
		setTimeout(() => {
			if (mines === hidden) {
				...
				msgBox.build(
					...
					[
						'lime',
						`Size: ${field.sizeX}x${field.sizeX}`,
						'yellow',
						`Time: ${mins}:${secs}:${millis}`,
						'red',
	...
	function redrawAdjusted(x, y, type) {
		...
		if (type === 'octal') {
			...
			if (!zeroList.length) {
				...
				zeroList = []
				for (const val of updateList)
					checkCell(val.x, val.y, 'zero', val.del)
	...
	function clickR() {
		table[x][y].data.type =
			table[x][y].data.type == 'block' ? 'flag' : 'block'
...
function redrawCell(x, y) {
	...
	for (let [a, b, i] of octangler(x, y)) {
		val[i] =
			val.center && cellExists(a, b) && table[a][b].data.type === val.type
	}
	...
	for (let i = 0; i < x; i++) {
		...
		for (let j = 0; j < y; j++) {
			...
			if (minefield[i][j] === 'mine')
				data = { center: true, type: 'block', value: '☀' }
			else
				data = {
					center: true,
					type: 'block',
...
msgBox.build(
	...
	[
		...
		],
		{
			...
			action: function () {
				let [x, y, diff] =
					msgBox.children.input.getElementsByClassName('number')
